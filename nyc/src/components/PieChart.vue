...
const pieOptions = computed(() => {
  ...
  const violationsByType = filteredViolations.reduce((accumulator, currentValue) => {
    ...
    accumulator[violationType] += 1
    return accumulator
  }, {})

  const totalViolations = Object.values(violationsByType).reduce((total, count) => total + count, 0)

  const seriesData = Object.values(violationsByType)
  const labels = Object.keys(violationsByType).map((type) => {
    const count = violationsByType[type]
    const percentage = (count / totalViolations) * 100
...
const pieSeries = computed(() => {
  ...
  return filteredViolations.map((violation) => violation.violation)
})
</script>
